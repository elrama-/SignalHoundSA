...
def testDeviceStatusQueries(sh):
	print 'entered te def'
	sh.getFirmwareVersion()
	print "Firmware gotten"
	sh.getAPIVersion()
...
def test_sweeping(sh):
	print "Opening"
	sh.OpenDevice()
	print "Configuring center and span"
	sh.configureCenterSpan(center=900.0e6, span=1.0e6)
	print "Configuring Acquisiton"
	sh.configureAcquisition(detector="min-max", scale="log-scale")
	print "Configuring Level"
	sh.configureLevel(ref=-10,atte=0)
	print "Configuring"
