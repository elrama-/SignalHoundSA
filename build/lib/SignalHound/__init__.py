...
class SignalHound(object):
	...
	def openDevice(self):
		...
		if ret != hf.saNoError:
			if ret == hf.saNullPtrErr:
				raise ValueError("Could not open device due to null-pointer error!")
			elif ret == hf.saDeviceNotOpenErr:
				raise ValueError("Could not open device!")
			else:
				# raise ValueError("Could not open device due to unknown reason! %d")
	...
	def getDeviceDiagnostics(self):
		...
		Raw call ``sa_API saStatus saGetDeviceDiagnostics(int device, float *temperature, float *voltage1_8, float *voltage1_2, float *voltageUSB, float *currentUSB);``
		...
		err = self.dll.saQueryDiagnostics(self.deviceHandle, temperaturePnt, voltageUSBPnt, currentUSBPnt)
	...
	def queryStreamInfo(self):
		...
		Returns:
			dictionary containing status information on the IQ data stream:
				...
				| {
				|	"return_len":       <The number of IQ samples pairs which will be returned by calling ``saFetchRaw()``.>,
		...
		Raw call ``sa_API saStatus saQueryStreamInfo(int device, int *return_len, double *bandwidth, int *samples_per_sec);``
		...
		err = self.dll.saQueryStreamInfo(self.deviceHandle, return_lenPnt, bandwidthPnt, samples_per_secPnt)
	...
	def configureAcquisition(self, detector, scale):
		...
		Args:
			detectorType (string): Specifies the video detector. The two possible values for detector type
					are:
					 - "average" (mapped to ``sa_AVERAGE``)
					 - "min-max" (mapped to ``sa_MIN_MAX``).
			verticalScale (string):  Specifies the scale in which sweep results are returned int. The four
					possible values for verticalScale are:
					 - "log-scale" (mapped to ``sa_LOG_SCALE``)
					 - "lin-scale" (mapped to ``sa_LIN_SCALE``),
					 - "log-full-scale" (mapped to ``sa_LOG_FULL_SCALE``)
					 - "lin-full-scale" (mapped to ``sa_LIN_FULL_SCALE``)
		Returns:
			Nothing

		The verticalScale parameter will change the units of returned sweeps. If ``sa_LOG_SCALE`` is provided
		sweeps will be returned in amplitude unit dBm. If ``sa_LIN_SCALE`` is specified, the returned units will be in
		...
		averaging is chosen the min and max trace arrays returned from saFetchTrace will contain the same
		averaged data
		...
		Will raise ``IOError`` for the following error-codes:
		 - ``saDeviceNotOpenErr`` - "Device not open!"
		 - ``saInvalidDetectorErr`` - "Invalid Detector mode!"
		 - ``saInvalidScaleErr`` - "Invalid scale setting error!"
		 - Any unknown errors

		Raw call: ``sa_API saStatus saConfigureAcquisition(int device, unsigned int detector, unsigned int scale);``
		'''
		...
		detectorVals = {
			"min-max" : ct.c_uint(hf.sa_MIN_MAX),
			"average" : ct.c_uint(hf.sa_AVERAGE)
		}

		scaleVals = {
			"log-scale"      : ct.c_uint(hf.sa_LOG_SCALE),
			"lin-scale"      : ct.c_uint(hf.sa_LIN_SCALE),
			"log-full-scale" : ct.c_uint(hf.sa_LOG_FULL_SCALE),
			"lin-full-scale" : ct.c_uint(hf.sa_LIN_FULL_SCALE)
		}
	...
	def configureCenterSpan(self, center, span):
		...
		returned after initiation. Refer to the saQueryTraceInfo function for more information.

		Each device has a specified operational frequency range. These limits are ``sa#_MIN_FREQ`` and
		``sa#_MAX_FREQ``, where ``#`` is the signal-hound model number (see ``sa_api_h.py``). The center and
		...
		Raw call: ``sa_API saStatus saConfigureCenterSpan(int device, double center, double span);``
		'''
	...
	def configureLevel(self, ref, atten):
		...
		settings for an input signal with amplitude equal to reference level. If a gain other than sa_AUTO_GAIN
		is specified using saConfigureGain, the reference level parameter is ignored.
		...
		Raw call: ``sa_API saStatus saConfigureLevel(int device, double ref, double atten);``
		...
		err = self.dll.saConfigLevel(self.deviceHandle, ref, atten)
	...
	def configureGain(self, gain):
		...
		The gain choices for each device range from 0 to ``sa#_MAX_GAIN``, where ``#`` is the signal-hound model number (see ``sa_api_h.py``).

		When ``sa_AUTO_GAIN`` is selected, the API uses the reference level provided in saConfigureLevel to
		...
		Raw call: ``sa_API saStatus saConfigureGain(int device, int gain);``
		'''
		...
		err = self.dll.saConfigureGain(self.deviceHandle, gain)

		if err == self.saStatus["saNoError"]:
			self.log.info("Call to configureGain succeeded.")
		elif err == self.saStatus["saDeviceNotOpenErr"]:
			raise IOError("Device not open!")
		elif err == self.saStatus["saInvalidGainErr"]:
			raise IOError("The specified gain value is outside the range of possible gains. Valid values are 0-%d, or \"auto\"" % hf.sa_MAX_GAIN)
		else:
	...
	def configureSweepCoupling(self, rbw, vbw, sweepTime, rbwType, rejection):
		...
		Args
			...
			rbwType (float): The possible values for rbwType are sa_NATIVE_RBW and
					sa_NON_NATIVE_RBW. This choice determines which bandwidth table
					is used and how the data is processed. sa_NATIVE_RBW is default and
					unchangeable for real-time operation.
			rejection (float): The possible values for rejection are sa_NO_SPUR_REJECT,
					sa_SPUR_REJECT, and sa_BYPASS_RF.
		...
		Rejection can be used to optimize certain aspects of the signal. Default is sa_NO_SPUR_REJECT, and
		should be used in most cases. If you have a steady CW or slowly changing signal, and need to minimize
		image and spurious responses from the device, use sa_SPUR_REJECT. If you have a signal between 300
		MHz and 3 GHz, need the lowest possible phase noise, and do not need any image rejection,
		sa_BYPASS_RF can be used to rewire the front end for lowest phase noise.
		...
		Raw Call: ``sa_API saStatus saConfigureSweepCoupling(int device, double rbw, double vbw, double sweepTime, unsigned int rbwType, unsigned int rejection);``
		'''
		...
		rbwVals = {
			"native"     : ct.c_uint(hf.sa_NATIVE_RBW),
			"non-native" : ct.c_uint(hf.sa_NON_NATIVE_RBW)
		}

		rejectionVals = {
			"no-spur-reject" : ct.c_uint(hf.sa_NO_SPUR_REJECT),
			"spur-reject"    : ct.c_uint(hf.sa_SPUR_REJECT),
			"bypass"         : ct.c_uint(hf.sa_BYPASS_RF)
		}
		...
		err = self.dll.saConfigSweepCoupling(self.deviceHandle, rbw, vbw, sweepTime, rbwType, rejection)
	...
	def configureIQ(self, downsample, bandwidth):
		...
		filter rolloff. That table is above. See  saFetchRaw() for polling the IQ data stream

		Raw Call: ``sa_API saStatus saConfigureIQ(int device, int downsampleFactor, double bandwidth);``
		...
		err = self.dll.saConfigureIQ(self.deviceHandle, downsample, bandwidth)
		...
		elif err == self.saStatus["saClampedToLowerLimit"]:
			raise IOError("The bandpass filter width specified is lower than  sa_MIN_IQ_BW")
		elif err == self.saStatus["saClampedToUpperLimit"]:
			raise IOError("Warning that the bandpass filter width was clamped to the maximum value allowed by the downsampleFaction.")
		else:
			raise IOError("Unknown error setting saConfigureIQ! Error = %s" % err)
	...
	def configureWindow(self, window):
		'''
		Args:
			window:  The possible values for window are sa_NUTALL, sa_BLACKMAN,
					sa_HAMMING, and sa_FLAT_TOP.

		This changes the windowing function applied to the data before signal processing is performed. In real-
		time configuration the window parameter is permanently set to sa_NUTALL. The windows are only
		changeable when using the sa_NATIVE_RBW type in saConfigureSweepCoupling. When using
		sa_NON_NATIVE_RBWs, a custom flattop window will be used.

		Raw Call: ``sa_API saStatus saConfigureWindow(int device, unsigned int window);``
		'''
		...
		windows = {
			"nutall"   : hf.sa_NUTALL,
			"blackman" : hf.sa_BLACKMAN,
			"hamming"  : hf.sa_HAMMING,
			"flat-top" : hf.sa_FLAT_TOP
		}
		...
		err = self.dll.saConfigureWindow(self.deviceHandle, window)

		if err == self.saStatus["saNoError"]:
			self.log.info("Call to configureWindow succeeded.")
		elif err == self.saStatus["saDeviceNotOpenErr"]:
			raise IOError("Device not open!")
		elif err == self.saStatus["saInvalidWindowErr"]:
	...
	def configureProcUnits(self, units):
		...
		Args:
			units:  The possible values are sa_LOG, sa_VOLTAGE, sa_POWER, and
					sa_BYPASS.
		...
		For “average power” measurements, sa_POWER should be selected. For cleaning up an amplitude
		modulated signal, sa_VOLTAGE would be a good choice. To emulate a traditional spectrum analyzer,
		select sa_LOG. To minimize processing power, select sa_BYPASS.
		...
		Raw Call: ``sa_API saStatus saConfigureProcUnits(int device, unsigned int units);``
		'''
		...
		err = self.dll.saConfigureProcUnits(self.deviceHandle, units)

		if err == self.saStatus["saNoError"]:
			self.log.info("Call to configureProcUnits succeeded.")
		elif err == self.saStatus["saDeviceNotOpenErr"]:
			raise IOError("Device not open!")
		elif err == self.saStatus["saInvalidVideoUnitsErr"]:
	...
	def configureTrigger(self, trigType, edge, level, timeout):
		...
		Args:
			type:  Specifies the type of trigger to use. Possible values are
				sa_NO_TRIGGER, sa_VIDEO_TRIGGER, sa_EXTERNAL_TRIGGER, and
				sa_GPS_PPS_TRIGGER. If an external signal is desired, BNC port 2 must
				be configured to accept a trigger (see saConfigureIO). When
				sa_NO_TRIGGER is specified, the other parameters are ignored and this
				function sets only trigger type.
			edge:  Specifies the edge type of a video trigger. Possible values are
				sa_TRIGGER_RISING and sa_TRIGGER_FALLING. If you are using a
		...
		Raw Call: ``sa_API saStatus saConfigureTrigger(int device, unsigned int type, unsigned int edge, double level, double timeout);``
		'''
		...
		videoTrigDict = {
			"rising-edge"  : hf.sa_TRIGGER_RISING,
			"falling-edge" : hf.sa_TRIGGER_FALLING
		}

		if trigType == "none":
			trigType =  hf.sa_NO_TRIGGER
		elif trigType == "video":
			trigType = hf.sa_VIDEO_TRIGGER
		elif trigType == "external":
			trigType = hf.sa_EXTERNAL_TRIGGER
		...
		if trigType == hf.sa_VIDEO_TRIGGER:
			...
			# Trigger type is ignored in all modes except sa_VIDEO_TRIGGER
			edge =  hf.sa_TRIGGER_FALLING
		...
		err = self.dll.saConfigureTrigger(self.deviceHandle, trigType, edge, level, timeout)
	...
	def configureTimeGate(self, delay, length, timeout):
		...
		Therefore it is necessary to use saConfigureIO to setup an external trigger.

		Raw Call: ``sa_API saStatus saConfigureTimeGate(int device, double delay, double length, double timeout);``
		'''
		...
		err = self.dll.saConfigureTimeGate(self.deviceHandle, delay, length, timeout)
	...
	def configureRawSweep(self, start, ppf, steps):
		'''
		Raw Call: ``sa_API saStatus saConfigureRawSweep(int device, int start, int ppf, int steps, int stepsize);``
		...
		Args:
			...
			stepsize:  Value must be sa_TWENTY_MHZ

		This function configures the device for both sa_RAW_SWEEP and sa_RAW_SWEEP_LOOP modes. This
		...
		stepSize = ct.c_int(20000000)

		err = self.dll.saConfigureRawSweep(self.deviceHandle, start, ppf, steps, stepSize)
	...
	def configureIO(self, port1Coupling, port1mode, port2mode):
		...
		Args:
			port1:  The first BNC port may be used to input or output a 10 MHz time base
				...
				port are found in the header file and are prefixed with “sa_PORT1”
		...
		combine a coupled type and a port type.
		 ...
		 - ``sa_PORT1_AC_COUPLED``               Denotes an AC coupled port
		 - ``sa_PORT1_DC_COUPLED``               Denotes a DC coupled port
		 - ``sa_PORT1_INT_REF_OUT``              Output the internal 10 MHz timebase
		 - ``sa_PORT1_EXT_REF_IN``               Accept an external 10MHz time base
		 - ``sa_PORT1_OUT_LOGIC_LOW``            Self-explanitory
		 - ``sa_PORT1_OUT_LOGIC_HIGH``           Self-explanitory

		Port 2 IO
		 - ``sa_PORT2_OUT_LOGIC_LOW``            Self-explanitory
		 - ``sa_PORT2_OUT_LOGIC_HIGH``           Self-explanitory
		 - ``sa_PORT2_IN_TRIGGER_RISING_EDGE``   When set, the device is notified of a rising edge
		 - ``sa_PORT_IN_TRIGGER_FALLING_EDGE``   When set, the device is notified of a falling edge

		Raw Call: ``sa_API saStatus saConfigureIO(int device, unsigned int port1, unsigned int port2);``
		'''
		...
		p1CouplingOpts = {
			"ac": hf.sa_PORT1_AC_COUPLED,
			"dc": hf.sa_PORT1_DC_COUPLED
		}

		p1ModeOpts = {
			"int-ref-out"    : hf.sa_PORT1_INT_REF_OUT,
			"ext-ref-in"     : hf.sa_PORT1_EXT_REF_IN,
			"out-logic-low"  : hf.sa_PORT1_OUT_LOGIC_LOW,
			"out-logic-high" : hf.sa_PORT1_OUT_LOGIC_HIGH
		}

		p2ModeOpts = {
			"int-ref-out"    : hf.sa_PORT2_IN_TRIGGER_RISING_EDGE,
			"ext-ref-in"     : hf.sa_PORT2_IN_TRIGGER_FALLING_EDGE,
			"out-logic-low"  : hf.sa_PORT2_OUT_LOGIC_LOW,
			"out-logic-high" : hf.sa_PORT2_OUT_LOGIC_HIGH
		}
		...
		err = self.dll.saConfigureIO(self.deviceHandle, port1, port2)

		if err == self.saStatus["saNoError"]:
			self.log.info("Call to configureIO succeeded.")
		elif err == self.saStatus["saDeviceNotOpenErr"]:
			raise IOError("Device not open!")
		elif err == self.saStatus["saDeviceNotIdleErr"]:
	...
	def configureDemod(self, modulationType, freq, ifBw, audioLowPassFreq, audioHighPassFreq, fmDeemphasis):
		...
		Args:
			modulationType:  Specifies the demodulation scheme, possible values are
					sa_DEMOD_AM/FM/Upper sideband (USB)/Lower Sideband (LSB)/CW.
		...
		Raw Call: ``sa_API saStatus saConfigureDemod(int device, int modulationType, double freq, float IFBW, float audioLowPassFreq, float audioHighPassFreq, float FMDeemphasis);``
		'''
		...
		err = self.dll.saConfigureDemod(self.deviceHandle, modulationType, freq, ifBw, audioLowPassFreq, audioHighPassFreq, fmDeemphasis)
	...
	def initiate(self, mode, flag=0, gps_timestamp=False):
		'''
		Args:
			mode:  The possible values for mode are sa_SWEEPING, sa_REAL_TIME,
				sa_ZERO_SPAN, sa_TIME_GATE, sa_RAW_SWEEP,
				sa_RAW_SWEEP_LOOP and sa_AUDIO_DEMOD.
			flag:  The default value is zero.
				If mode equals sa_ZERO_SPAN, flag can be used to denote the type of
				modulation performed on the incoming signal. sa_DEMOD_AM and
				sa_DEMOD_FM are the two options.
				Flag can be used to inform the API to time
				stamp data using an external GPS reciever. Mask the bandwidth flag (‘|’
				in C) with sa_TIME_STAMP to achieve this. See Appendix:Using a GPS
				Receiver to Time-Stamp Data for information on how to set this up.
			gps_timestamp (bool): Timestamp using GPS

		saInitiate configures the device into a state determined by the mode parameter. For more information
		regarding operating states, refer to the Theory of Operation and Modes of Operation sections. This
		function calls saAbort before attempting to reconfigure. It should be noted, if an error is returned, any
		past operating state will no longer be active.

		Pay special attention to the saInvalidParameterErr description below

		Raw Call: ``sa_API saStatus saInitiate(int device, unsigned int mode, unsigned int flag);``
		'''
		...
		modeOpts = {
			"sweeping"       : hf.sa_SWEEPING,
			"real_time"      : hf.sa_REAL_TIME,
			"IQ"             : hf.sa_IQ,
			"idle"           : hf.sa_IDLE
		}
...
#there was some job from real time and zero span modes, erased!
		if gps_timestamp:
			self.log.info("Timestamping returned data with GPS time")
			flag |= hf.sa_TIME_STAMP
	...
	def fetchTrace(self):
		...
		detectorType provided in saConfigureAcquisition is sa_AVERAGE, the arrays will contain identical
		values. Element zero of each array corresponds to the startFreq returned from saQueryTraceInfo.

		Raw Call: ``sa_API saStatus saFetchTrace(int device, int arraySize, double *min, double *max);``
		'''
		...
		err = self.dll.saFetchTrace(self.deviceHandle, arraySize, minPtr, maxPtr)
		...
		elif err == self.saStatus["saBufferTooSmallErr"]:
			raise IOError("The 'arraySize' parameter passed is less than the trace size returned from 'saQueryTraceInfo'.")
		elif err == self.saStatus["saADCOverflow"]:
		...
		elif err == self.saStatus["saNoTriggerFound"]:
	...
	def fetchAudio(self):
		...
		Raw Call: ``sa_API saStatus saFetchAudio(int device, float *audio);``
		'''
		...
		err = self.dll.saFetchAudio(self.deviceHandle, audioArrPtr)
	...
	def fetchRawCorrections(self):
		...
		requested. The correction array will only be generated again on a new saInitiate().
		...
		Raw Call: ``sa_API saStatus saFetchRawCorrections(int device, float *corrections, int *index, double *startFreq);``
		'''
		...
		err = self.dll.saFetchRawCorrections(self.deviceHandle, corrArrPtr, indexPtr, startFreqPtr)
	...
	def fetchRawSweep(self):
		...
		Raw Call: ``sa_API saStatus saFetchRawSweep(int device, short *buffer);``
		'''
		...
		err = self.dll.saFetchRawSweep(self.deviceHandle, rawBufPtr)
	...
	def startRawSweepLoop(self, callbackFunc):
		...
		The device sweeps indefinitely until saAbort or saCloseDevice is called. When operation is suspended
		via saAbort, the device must be reconfigured and initiated again before calling this function.

		Raw Call: ``sa_API saStatus saStartRawSweepLoop(int device, void(*sweep_callback)(short *buffer, int len));``
		'''
		...
		err = self.dll.saStartRawSweepLoop(self.deviceHandle, self.cRawSweepCallbackFunc)
	...
	def queryTraceInfo(self):
		...
		Returns:
			| ``dict`` containing:
			|	"arr-size": The size of arrays returned by saFetchTrace.
		...
		Note: Calling while in sa_RAW_PIPE mode will produce a saDeviceNotConfiguredErr

		Raw Call: ``sa_API saStatus saQueryTraceInfo(int device, unsigned int *traceLen, double *binSize, double *start);``
		'''
		...
		err = self.dll.saQueryTraceInfo(self.deviceHandle, traceLenPnt, binSizePnt, startPnt)
	...
	def queryStreamingCenter(self):
		...
		Raw Call: ``sa_API saStatus saQueryStreamingCenter(int device, double *center);``
		'''
		...
		err = self.dll.saQueryStreamingCenter(self.deviceHandle, centerPnt)
	...
	def queryTimestamp(self):
		...
		This function is used in conjunction with saSyncCPUtoGPS and a GPS device to retrieve an absolute time
		for a data packet in raw pipe mode. This function returns an absolute time for the last packet retrieved
		from saFetchRaw. See the Appendix:Code Examples for information on how to setup and interpret the
		time information.

		Raw Call: ``sa_API saStatus saQueryTimestamp(int device, unsigned int *seconds, unsigned int *nanoseconds);``
		'''
		...
		err = self.dll.saQueryTimestamp(self.deviceHandle, secondsPnt, nanosecondsPnt)
	...
	def abort(self):
		...
		Raw Call: ``sa_API saStatus saAbort(int device);``
		'''
	...
	def preset(self):
		...
		after 2 seconds. Within this time you must call saCloseDevice to free any remaining resources and
		release the device serial number from the open device list. From the time of the saPreset call, we
		suggest 3 to more seconds of wait time before attempting to re-open the device.

		Raw Call: ``sa_API saStatus saPreset(int device);``
		'''
	...
	def selfCal(self):
		...
		saAbort() to suspend all device operations before performing the calibration, and will return the device
		...
		shape/amplitude of the IF. We suggest using saQueryDiagnostics to monitor the device’s temperature
		...
		device in calibrated. The device is calibrated once upon opening the device through saOpenDevice and is
		...
		prior to the calling of saSelfCal(). Fully reconfiguring the device will be necessary.

		Raw Call: ``sa_API saStatus saSelfCal(int device);``
		'''

		self.log.info("Performing self-calibration of device.")

		err = self.dll.saSelfCal(self.deviceHandle)
	...
	def syncCPUtoGPS(self, comPort, baudRate):
		...
		Raw Call: ``sa_API saStatus saSyncCPUtoGPS(int comPort, int baudRate);``
		'''
		...
		err = self.dll.saSyncCPUtoGPS(comPort, baudRate)

		if err == self.saStatus["saNoError"]:
			self.log.info("Call to syncCPUtoGPS succeeded.")
		elif err == self.saStatus["saDeviceNotOpenErr"]:
			raise IOError("Device not open!")
		elif err == self.saStatus["saGPSErr"]:
	...
	def getDeviceType(self):
		...
		Raw Call: ``sa_API saStatus saGetDeviceType(int device, int *type);``
		'''
		...
		if devType.value == hf.saDeviceTypeNone:
			dev = "No device"
		elif devType.value == hf.saDeviceTypeSA44:
			dev = "sa44"
		elif devType.value == hf.saDeviceTypeSA44B:
			dev = "sa44B"
		elif devType.value == hf.saDeviceTypeSA124A:
			dev = "sa124A"
		elif devType.value == hf.saDeviceTypeSA124B:
			dev = 'sa124B'
		else:
	...
	def getSerialNumber(self):
		...
		Raw Call: ``sa_API saStatus saGetSerialNumber(int device, unsigned int *sid);``
		'''
		...
		err = self.dll.saGetSerialNumber(self.deviceHandle, serialNoPnt)
	...
	def getFirmwareVersion(self):
		...
		Raw Call: ``sa_API saStatus saGetFirmwareVersion(int device, int *version);``
		'''
		...
		err = self.dll.saGetFirmwareString(self.deviceHandle, firmwareRevPnt)
	...
	def getAPIVersion(self):
		...
		Raw Call: ``sa_API const char* saGetAPIVersion();``
		'''


		self.log.info("Querying API for revision information.")

		self.dll.saGetAPIVersion.restype = ct.c_char_p  # Tell ctypes this function returns a pointer to a string
		apiRevStr = self.dll.saGetAPIVersion(self.deviceHandle)
	...
	def getErrorString(self, errCode):
		...
		This /should/ be of type saStatus. saStatus is an enum with hard-coded values, so I'm being lazy, and just using
		an int. It works well enough.

		Raw Call: ``sa_API const char* saGetErrorString(saStatus status);``
		'''

		serialNo = ct.c_int(errCode)

		self.dll.saGetAPIVersion.restype = ct.c_char_p  # Tell ctypes this function returns a pointer to a string
		apiRevStr = self.dll.saGetAPIVersion(self.deviceHandle, serialNo)
	...
	# methods written by Ramiro
	def configureSweepCoupling_2(self, rbw, vbw, rejection):
		self.log.info("Setting device sweep coupling settings.")
		...
		err = self.dll.saConfigSweepCoupling(self.deviceHandle, rbw, vbw, reject_var)
	...
	def QuerySweepInfo(self):
		self.log.info("Querying the Sweep configuration.")

		sweeplength       = ct.c_int(0)
		startfreq         = ct.c_double(0)
		binsize           = ct.c_double(0)

		sweeplengthPnt  = ct.pointer(sweeplength)
		startfreqPnt  = ct.pointer(startfreq)
		binsizePnt  = ct.pointer(binsize)

		err = self.dll.saQuerySweepInfo(self.deviceHandle, sweeplengthPnt, startfreqPnt, binsizePnt)
		...
		self.buffer_length = sweeplength.value

		values = {
			"sweep_length"   : sweeplength.value,
			"start_freq"     : startfreq.value,
			"bin_size"       : binsize.value
		}

		return values

	def GetSweep_32f(self):
		self.log.info("Starting the sweep with 32-bit buffers.")

		bufLen = self.buffer_length
		bufmax = (ct.c_float * bufLen)()
		bufmaxPtr = ct.pointer(bufmax)
		bufmin = (ct.c_float * bufLen)()
		bufminPtr = ct.pointer(bufmin)

		err = self.dll.saGetSweep_32f(self.deviceHandle, bufminPtr, bufmaxPtr)
		...
		datamin = SignalHound.fastDecodeArray(bufmax,bufLen, np.float,np.dtype(float))
		datamax = SignalHound.fastDecodeArray(bufmin,bufLen, np.float,np.dtype(float))
		array   = np.array([datamin,datamax])
		return array

	def GetSweep_64f(self):
		self.log.info("Starting the sweep with 32-bit buffers.")

		bufLen = self.buffer_length
		bufmax = (ct.c_double * bufLen)()
		bufmaxPtr = ct.pointer(bufmax)
		bufmin = (ct.c_double * bufLen)()
		bufminPtr = ct.pointer(bufmin)

		err = self.dll.saGetSweep_64f(self.deviceHandle, bufminPtr, bufmaxPtr);
		...
		datamin = SignalHound.fastDecodeArray(bufmax,bufLen, np.double,np.dtype(double))
		datamax = SignalHound.fastDecodeArray(bufmin,bufLen, np.double,np.dtype(double))
		array   = np.array([datamin,datamax])
		return array

	def EnableExternalRef(self):
...
##########################################################################################################
	...
	@staticmethod
	def fastDecodeArray(ctBuff, buffLen, dtype,vartype):
		'''
		Args:
			bufPtr (pointer to buffer): Pointer to a C buffer containing a sweep dataset
			buflen (integer buffer size): Size of the data in ``bufPtr``
		...
		buffer = buffer_from_memory(bufAdr, vartype.itemsize*buffLen)
		arr = numpy.frombuffer(buffer, dtype=dtype)  # Map array memory as a numpy array.
		arr = arr.copy()  # Then copy it, so our array won't get modified when the circular buffer overwrites itself.
		# We have to copy() since the call normally just returns a array that is overlaid onto the pre-existing data
		return arr
